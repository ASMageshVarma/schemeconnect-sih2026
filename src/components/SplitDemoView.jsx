...
import { SchemeConnectFeed } from './SchemeConnectFeed';

export function SplitDemoView({ lang, t, onOpenCalculator, onOpenLocator, onOpenCounselor }) {
  return (
    <div className="w-full min-h-screen bg-slate-950 p-3 sm:p-5 text-slate-100 animate-fadeIn flex flex-col">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 mb-4 shadow-xl flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span className="text-xs font-black uppercase text-amber-300 tracking-wider">
                Hackathon Judge Live Demo Mode
              </span>
        ...
        {/* Demo Guide Pill */}
        <div className="text-xs text-slate-300 bg-slate-800/80 px-4 py-2 rounded-2xl border border-slate-700 flex items-center gap-2">
          <Zap className="w-4 h-4 text-amber-400 shrink-0" />
          <span>
            <b>Demo Step:</b> Click <span className="text-emerald-300 font-bold">"Extend NSFDC Age to 40"</span> on the Left ➔ Watch NSFDC Micro unlock on the Right!
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1">
        ...
        <div className="lg:col-span-6 bg-slate-900/90 rounded-3xl border border-indigo-900/60 p-4 sm:p-5 overflow-y-auto max-h-[85vh] shadow-2xl">
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-800">
            <div className="flex items-center space-x-2">
              <span className="w-3 h-3 rounded-full bg-indigo-500"></span>
              <span className="text-xs font-black uppercase tracking-wider text-indigo-300">
                Left Panel: Alpha Portal Government Console
              </span>
            </div>
        ...
        {/* RIGHT 6 COLS: SCHEMECONNECT CITIZEN FEED */}
        <div className="lg:col-span-6 bg-slate-100 text-slate-900 rounded-3xl border border-slate-700 p-4 sm:p-5 overflow-y-auto max-h-[85vh] shadow-2xl">
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-200">
            <div className="flex items-center space-x-2">
              <span className="w-3 h-3 rounded-full bg-emerald-500"></span>
              <span className="text-xs font-black uppercase tracking-wider text-slate-900">
                Right Panel: SchemeConnect Citizen Feed (Rajan S. 39 Yrs)
              </span>
            </div>
